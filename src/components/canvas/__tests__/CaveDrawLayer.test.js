...
describe('CaveDrawLayer', () => {
    it('renders nothing when no points', () => {
        const { container } = render(_jsx(CaveDrawLayer, { points: [], mousePos: null }));
        expect(container.firstChild).toBeNull();
    });
    it('renders a layer when points provided', () => {
        const { getByTestId } = render(_jsx(CaveDrawLayer, { points: [0, 0, 50, 0, 50, 50], mousePos: null }));
        expect(getByTestId('layer')).toBeInTheDocument();
    });
    it('renders vertex circles for each point pair', () => {
        const { getAllByTestId } = render(_jsx(CaveDrawLayer, { points: [0, 0, 50, 0, 50, 50], mousePos: null }));
        // 3 vertices → 3 circles
        expect(getAllByTestId('circle')).toHaveLength(3);
    });
    it('renders preview line when mousePos is provided', () => {
        const { getAllByTestId } = render(_jsx(CaveDrawLayer, { points: [0, 0, 50, 50], mousePos: { x: 100, y: 100 } }));
    ...
    it('does not render preview line when mousePos is null', () => {
        ...
        const { getAllByTestId } = render(_jsx(CaveDrawLayer, { points: [0, 0], mousePos: null }));
        const lines = getAllByTestId('line');
        // Only the open outline line, no closed polygon, no preview
        expect(lines).toHaveLength(1);
    });
    it('renders filled polygon when 4+ point values given', () => {
        const { getAllByTestId } = render(_jsx(CaveDrawLayer, { points: [0, 0, 50, 0, 50, 50, 0, 50], mousePos: null }));
...
describe('PaintCaveDrawLayer', () => {
    it('renders nothing when fewer than 4 point values', () => {
        const { container } = render(_jsx(PaintCaveDrawLayer, { points: [0, 0] }));
        expect(container.firstChild).toBeNull();
    });
    it('renders nothing for empty points', () => {
        const { container } = render(_jsx(PaintCaveDrawLayer, { points: [] }));
        expect(container.firstChild).toBeNull();
    });
    it('renders layer with lines when 4+ point values provided', () => {
        const { getByTestId } = render(_jsx(PaintCaveDrawLayer, { points: [0, 0, 10, 10, 20, 0, 30, 10] }));
        expect(getByTestId('layer')).toBeInTheDocument();
    });
});
