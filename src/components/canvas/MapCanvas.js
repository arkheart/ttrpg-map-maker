...
export const MapCanvas = forwardRef(function MapCanvas({ selectedElement, onSelect }, ref) {
    ...
    useEffect(() => {
        const onKeyDown = (e) => { if (e.key === 'Control')
            setCtrlHeld(true); };
        const onKeyUp = (e) => { if (e.key === 'Control') {
            setCtrlHeld(false);
    ...
    const handleMouseMove = (e) => {
        ...
        const isPolygonMode = (activeTool === 'cave' && caveDrawMode === 'polygon') ||
    ...
    const handleClick = (e) => {
        ...
        if (activeTool === 'room' && roomDrawMode === 'custom') {
            if (isNearFirst(roomPoints, pos.x, pos.y))
                commitRoomCustom(roomPoints);
            else
                setRoomPoints(prev => [...prev, pos.x, pos.y]);
            return;
        }
        ...
        if ((activeTool === 'select' || activeTool === 'edit') && onStage) {
            onSelect(null);
            return;
        }
    ...
    const handleMouseDown = (e) => {
        ...
        const isDragTool = (activeTool === 'room' && (roomDrawMode === 'rect' || roomDrawMode === 'ellipse')) ||
    ...
    const handleSelect = (type, id) => {
        if (activeTool === 'erase')
            dispatch({ type: 'DELETE_ELEMENT', payload: { id } });
        else if (activeTool === 'select' || activeTool === 'edit')
            onSelect({ type, id });
    };
        ...
        exportPng() {
            ...
            for (const r of state.rooms) {
                if (r.shape === 'rect' || !r.shape) {
                    ...
                    minX = Math.min(minX, r.x - r.radiusX);
                    minY = Math.min(minY, r.y - r.radiusY);
                    maxX = Math.max(maxX, r.x + r.radiusX);
                    maxY = Math.max(maxY, r.y + r.radiusY);
                }
            ...
            for (const t of state.terrain) {
                if (t.shape === 'rect') {
                    ...
                    minX = Math.min(minX, t.x - t.radiusX);
                    minY = Math.min(minY, t.y - t.radiusY);
                    maxX = Math.max(maxX, t.x + t.radiusX);
                    maxY = Math.max(maxY, t.y + t.radiusY);
                }
                else if (t.shape === 'custom') {
                    for (let i = 0; i < t.points.length; i += 2) {
                        minX = Math.min(minX, t.points[i]);
                        maxX = Math.max(maxX, t.points[i]);
                        minY = Math.min(minY, t.points[i + 1]);
                        maxY = Math.max(maxY, t.points[i + 1]);
                    }
                }
            }
    ...
    const showHint = (activeTool === 'cave' && caveDrawMode === 'polygon') ||
        (activeTool === 'cave' && caveDrawMode === 'paint') ||
        (activeTool === 'room' && roomDrawMode === 'custom') ||
        (activeTool === 'terrain' && terrainDrawMode === 'custom');
    const activePoints = activeTool === 'cave' ? cavePoints :
