...
const room = { id: 'r1', shape: 'rect' as const, x: 0, y: 0, width: 100, height: 80, fill: '#333', label: 'Entry Hall' }
const cave = { id: 'c1', points: [0, 0, 10, 10], fill: '#555', label: 'Dark Cave' }
const terrain = { id: 't1', shape: 'rect' as const, x: 0, y: 0, width: 50, height: 50, terrainType: 'forest' as const }
const item = { id: 'i1', x: 5, y: 5, symbol: '🚪', label: 'Main Door' }

const stateWithAll: Partial<MapState> = {
  rooms: [room],
  caves: [cave],
  terrain: [terrain],
  items: [item],
...
describe('ObjectsPanel', () => {
  it('renders Objects heading', () => {
    renderWithContext(<ObjectsPanel selected={null} onSelect={() => {}} onClose={() => {}} />)
  ...
  it('calls onClose when ✕ is clicked', async () => {
    const onClose = vi.fn()
    renderWithContext(<ObjectsPanel selected={null} onSelect={() => {}} onClose={onClose} />)
  ...
  it('renders all objects from layerOrder', () => {
    renderWithContext(
      <ObjectsPanel selected={null} onSelect={() => {}} onClose={() => {}} />,
      { mapState: stateWithAll }
    )
  ...
  it('calls onSelect with correct element when label is clicked', async () => {
    ...
    renderWithContext(
      <ObjectsPanel selected={null} onSelect={onSelect} onClose={() => {}} />,
      { mapState: stateWithAll }
    )
  ...
  it('highlights selected element', () => {
    renderWithContext(
      <ObjectsPanel selected={{ type: 'room', id: 'r1' }} onSelect={() => {}} onClose={() => {}} />,
      { mapState: stateWithAll }
    )
  ...
  it('dispatches REORDER_ELEMENT up when ▲ is clicked', async () => {
    ...
    renderWithContext(
      <ObjectsPanel selected={null} onSelect={() => {}} onClose={() => {}} />,
      { mapState: stateWithAll, dispatch }
    )
    ...
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'REORDER_ELEMENT', payload: expect.objectContaining({ direction: 'up' }) })
    )
  })
  ...
  it('dispatches REORDER_ELEMENT down when ▼ is clicked', async () => {
    ...
    const enabledDown = downButtons.find(b => !(b as HTMLButtonElement).disabled)
    await userEvent.click(enabledDown!)
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'REORDER_ELEMENT', payload: expect.objectContaining({ direction: 'down' }) })
    )
  })
