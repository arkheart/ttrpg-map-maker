...
const room = { id: 'r1', shape: 'rect' as const, x: 0, y: 0, width: 100, height: 80, fill: '#3a3a3a', label: 'Boss Room' }
const cave = { id: 'c1', points: [0, 0, 10, 10, 20, 0], fill: '#555', label: 'Hidden Cave' }
const terrain = { id: 't1', shape: 'rect' as const, x: 0, y: 0, width: 50, height: 50, terrainType: 'forest' as const, label: 'Dark Woods' }
...
describe('PropertiesPanel — room', () => {
  it('renders label input with existing label', () => {
    renderWithContext(
      <PropertiesPanel selected={{ type: 'room', id: 'r1' }} />,
  ...
  it('dispatches UPDATE_ROOM on label change', async () => {
    ...
    renderWithContext(
      <PropertiesPanel selected={{ type: 'room', id: 'r1' }} />,
      { mapState: { ...emptyMapState, rooms: [room], layerOrder: ['r1'] }, dispatch }
    )
    const input = screen.getByPlaceholderText('Room name...')
    fireEvent.change(input, { target: { value: 'Throne Room' } })
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_ROOM', payload: expect.objectContaining({ id: 'r1', label: 'Throne Room' }) })
    )
  })
  ...
  it('dispatches UPDATE_ROOM on preset color click', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_ROOM', payload: expect.objectContaining({ fill: '#555' }) })
    )
  })
  ...
  it('returns null when room id not found', () => {
    const { container } = renderWithContext(
      <PropertiesPanel selected={{ type: 'room', id: 'unknown' }} />,
...
describe('PropertiesPanel — cave', () => {
  it('renders label input with existing label', () => {
    renderWithContext(
      <PropertiesPanel selected={{ type: 'cave', id: 'c1' }} />,
      { mapState: { ...emptyMapState, caves: [cave], layerOrder: ['c1'] } }
    )
  ...
  it('dispatches UPDATE_CAVE on label change', async () => {
    ...
    renderWithContext(
      <PropertiesPanel selected={{ type: 'cave', id: 'c1' }} />,
      { mapState: { ...emptyMapState, caves: [cave], layerOrder: ['c1'] }, dispatch }
    )
    const input = screen.getByPlaceholderText('Cave name...')
    fireEvent.change(input, { target: { value: 'Dragon Lair' } })
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_CAVE', payload: expect.objectContaining({ id: 'c1', label: 'Dragon Lair' }) })
    )
  })

  it('returns null when cave id not found', () => {
    const { container } = renderWithContext(
      <PropertiesPanel selected={{ type: 'cave', id: 'unknown' }} />,
      { mapState: { ...emptyMapState } }
    )
    expect(container.firstChild).toBeNull()
  })
...
describe('PropertiesPanel — terrain', () => {
  it('renders terrain type buttons', () => {
    renderWithContext(
      <PropertiesPanel selected={{ type: 'terrain', id: 't1' }} />,
      { mapState: { ...emptyMapState, terrain: [terrain], layerOrder: ['t1'] } }
    )
  ...
  it('dispatches UPDATE_TERRAIN on terrain type click', async () => {
    ...
    renderWithContext(
      <PropertiesPanel selected={{ type: 'terrain', id: 't1' }} />,
      { mapState: { ...emptyMapState, terrain: [terrain], layerOrder: ['t1'] }, dispatch }
    )
    await userEvent.click(screen.getByText('Water'))
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_TERRAIN', payload: expect.objectContaining({ terrainType: 'water' }) })
    )
  })
  ...
  it('dispatches UPDATE_TERRAIN on label change', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_TERRAIN', payload: expect.objectContaining({ id: 't1', label: 'Enchanted' }) })
    )
  })
})
...
describe('PropertiesPanel — item', () => {
  it('renders symbol buttons', () => {
    renderWithContext(
      <PropertiesPanel selected={{ type: 'item', id: 'i1' }} />,
      { mapState: { ...emptyMapState, items: [item], layerOrder: ['i1'] } }
    )
  ...
  it('dispatches UPDATE_ITEM on symbol click', async () => {
    ...
    renderWithContext(
      <PropertiesPanel selected={{ type: 'item', id: 'i1' }} />,
      { mapState: { ...emptyMapState, items: [item], layerOrder: ['i1'] }, dispatch }
    )
    await userEvent.click(screen.getByTitle('Chest'))
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_ITEM', payload: expect.objectContaining({ symbol: '📦' }) })
    )
  })
  ...
  it('dispatches UPDATE_ITEM on label change', async () => {
    ...
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_ITEM', payload: expect.objectContaining({ id: 'i1', label: 'Side Entrance' }) })
    )
  })

  it('returns null when item id not found', () => {
    const { container } = renderWithContext(
      <PropertiesPanel selected={{ type: 'item', id: 'unknown' }} />,
      { mapState: { ...emptyMapState } }
    )
    expect(container.firstChild).toBeNull()
  })
...
describe('PropertiesPanel — GridSection', () => {
  it('shows Disabled when grid not enabled', () => {
    renderWithContext(
      <PropertiesPanel selected={{ type: 'room', id: 'r1' }} />,
      { mapState: { ...emptyMapState, rooms: [{ ...room, grid: { enabled: false, size: 32 } }], layerOrder: ['r1'] } }
    )
  ...
  it('shows Enabled and size/color/opacity inputs when grid enabled', () => {
    renderWithContext(
      <PropertiesPanel selected={{ type: 'room', id: 'r1' }} />,
      { mapState: { ...emptyMapState, rooms: [{ ...room, grid: { enabled: true, size: 32, color: '#fff', opacity: 0.2 } }], layerOrder: ['r1'] } }
    )
  ...
  it('dispatches UPDATE_ROOM with enabled grid when toggle clicked', async () => {
    ...
    renderWithContext(
      <PropertiesPanel selected={{ type: 'room', id: 'r1' }} />,
      { mapState: { ...emptyMapState, rooms: [{ ...room, grid: { enabled: false, size: 32 } }], layerOrder: ['r1'] }, dispatch }
    )
    await userEvent.click(screen.getByText('Disabled'))
    expect(dispatch).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'UPDATE_ROOM', payload: expect.objectContaining({ grid: expect.objectContaining({ enabled: true }) }) })
    )
  })
})
